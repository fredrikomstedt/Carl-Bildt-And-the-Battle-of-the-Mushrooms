...
public class Robotron extends JFrame
{
	...
	public static void main(String[] args) throws InterruptedException
	{
		...
		while(true)
		{
			...
			while(area.player.getLife() > 0)// Play if player isn't dead
			{
				...
				// Check whether the level is complete and it's time for the
				// next level, AKA when all enemies that can be killed are dead
				if(nextLevel())
				{
					break;
				}
				...
				// Collision checking for humans, enemies, bullets and the
				// player
				...
				shootingTimer = bulletCreation(shootingTimer);// Creation of
																// both enemy
																// and friendly
																// bullets,
																// using a timer
				...
				movement();// Movement for bullets, humans, enemies and the
							// player, as well as checking if they've moved
							// outside the screen

				area.repaint();// Put everything done this frame on the screen
			...
			{
				break;
			} else if(!playerHit)// Time for new level
			{
				levelStart = false;
	...
	public static void startLevel() throws InterruptedException
	{
		...
		// Creation of all different kinds of enemies, depending on which wave
		// it is
		if(wave > 3)
		{
		...
		// Show it to the player and give two seconds to see how the wave is
		// distributed
		area.repaint();
		Thread.sleep(2000);

		// spawn the player and give him/her the opportunity to look for 1
		// second on how to play, before the wave starts
	...
	{
		area.player.setLife(area.player.getLife() - 1);// Lower the health of
														// the player by 1

		area.bulletDestroyAll(area.bullet);// Destroy bullets so they don't
											// cause any harm right when the
											// wave restarts
		area.bulletDestroyAll(area.enemyBullet);// (Unfortunate to be hit by a
												// bullet right when you've
												// respawned)
		area.enemyHitPlayerAvoid();// Respawn the player and give another second
									// to look on how to play
		area.paintPlayer = true;
		area.player.edgesCheck();
		area.repaint();
		Thread.sleep(1000);
	...
	public static int bulletCreation(int shootingTimer)
	{
		...
		if(shootingTimer == 0)// Timer to make sure bullets are shot from player
								// in a constant flow, yet not all the time.
								// Enemy bullets are shot randomly
		{
			Listener.updateBulletShooting();// Shooting controlled by key
											// presses
			shootingTimer++;
			return shootingTimer;
		} else if(shootingTimer == 8)
		{
			shootingTimer = 0;
			return shootingTimer;
		} else
	...
	{
		area.paintPlayer = false;// Destroy everything and show Game Over
									// picture
		area.enemyDestroyAll();
		area.humanDestroyAll();
		area.bulletDestroyAll(area.bullet);
