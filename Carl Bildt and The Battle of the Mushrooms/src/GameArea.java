...
public class GameArea extends JPanel
{
	...
	public void paintComponent( Graphics g )//Draw everything onto the screen
	{
		...
		{
			g.drawImage( enemyBullet.get(i).getImage().getImage(), enemyBullet.get(i).getX(), enemyBullet.get(i).getY(), this );
		}
		...
		g.setColor( Color.DARK_GRAY );//Draw HUD, containing score and health left
		g.fillRect( 0, 0, getWidth(), 35 );
		g.setColor( Color.WHITE );
		g.setFont( new Font( "Comic Sans MS", Font.PLAIN, 25 ) );
		g.drawString( "Life: ", 10, 25 );
		g.setColor( Color.BLACK );
		for( int i = 0; i < player.getLife(); i++ )//Draw how many lives the player still has
		{
			g.drawImage( playerLife.getImage(), 80+48*i, 10, this );
		}
		g.setColor( Color.WHITE );
		g.drawString( "Score: " + Integer.toString( score ), getWidth() - 200, 25 );//Draw score
		g.drawString( "Wave: " + Integer.toString( Robotron.wave ), getWidth()/2-60, 25 );//Draw wave
		g.setColor( Color.BLACK );
		
		if( gameOver )//Draw Game Over screen if game is over
		{
			g.drawImage( gameOverPic.getImage(), getWidth()/2 - gameOverPic.getIconWidth()/2, getHeight()/2 - gameOverPic.getIconHeight()/2, this );
		}
	}
		...
		switch( type )
		{
			...
			case 2:
				...
				for( int i = 0; i < ( number + rand - 3 )/3; i++ )
				{
					...
					enemy.add( new Shooter( random.nextInt( getWidth() ), random.nextInt( getHeight() ), "enemy2.png", direction ) );
				}
				break;
		}
	}
	...
	public void humanCreate( int number )//Create humans
	{
		...
		{
			int temp = random.nextInt( 4 );//Give random starting direction to every human
			switch( temp )
			{
				case 0:
					direction = 0;
	...
	public void enemyBulletCreate()//Creation of enemy bullets
	{
		for( int i = 0; i < enemy.size(); i++ )
		{
			if( enemy.get(i) instanceof Shooter )
			{
				int direction = 0;
				Random random = new Random();
				int temp = random.nextInt( 75 );//1/75 chance every frame of every type 2 enemy to spawn a bullet in a random direction
				if( temp == 1 )
				{
					...
					switch( temp2 )
					{
						case 0:
							direction = 0;
							enemyBullet.add( new Bullet( enemy.get(i).getX() + 16, enemy.get(i).getY() + enemy.get(i).getHeight()/2, direction, "enemybullet.png" ) );
							break;
						case 1:
							direction = 90;
							enemyBullet.add( new Bullet( enemy.get(i).getX() + enemy.get(i).getWidth()/2, enemy.get(i).getY() - 16, direction, "enemybullet.png" ) );
							break;
						case 2:
							direction = 180;
							enemyBullet.add( new Bullet( enemy.get(i).getX() - 16, enemy.get(i).getY() + enemy.get(i).getHeight()/2, direction, "enemybullet.png" ) );
	...
	public void bulletDestroyOutside( LinkedList<Bullet> _bullet )//Destroy bullets outside of the screen
	{
	...
	public void bulletDestroyAll( LinkedList<Bullet> _bullet )//Destroy all bullets currently in game
	{
		for( int i = _bullet.size() - 1; i >= 0; i-- )
		{
			_bullet.remove(i);
	...
	public void bulletHitEnemyCheck()//Destroy colliding bullets and enemies and give points to the player
	{
		for( int i = bullet.size() - 1; i >= 0; i-- )
		{
			for( int ii = enemy.size() - 1; ii >= 0; ii-- )
			{
				if( bullet.get(i).getX() + (bullet.get(i).getWidth()/2) >= enemy.get(ii).getX() &&
					bullet.get(i).getX() + (bullet.get(i).getWidth()/2) <= enemy.get(ii).getX() + enemy.get(ii).getWidth() &&
					bullet.get(i).getY() + (bullet.get(i).getHeight()/2) >= enemy.get(ii).getY() &&
					bullet.get(i).getY() + (bullet.get(i).getHeight()/2) <= enemy.get(ii).getY() + enemy.get(ii).getHeight() &&
					!( enemy.get(ii) instanceof Lazy ) )
				{
					if( score < bonusLifeChecker*25000 && score + 100 >= bonusLifeChecker*25000 && player.getLife() < 5 )//Bonus lives are awarded every 25000 points
					{
						bonusLifeChecker++;
	...
	public void humanHitPlayerCheck()//Check if player saves humans and give points to the player if so
	{
		for( int i = human.size() - 1; i >= 0; i-- )
		{
			...
			{
				if( score < bonusLifeChecker*25000 && score + 2000 >= bonusLifeChecker*25000 && player.getLife() < 5 )//Bonus lives are awarded every 25000 points
				{
					bonusLifeChecker++;
					player.setLife( player.getLife() + 1 );
				}
	...
	public boolean enemyHitPlayerCheck()//Check if player has been hit by enemy
	{
		for( int i = enemy.size() - 1; i >= 0; i-- )
		{
			if( enemy.get(i).getX() >= player.getX() - 5 &&
				enemy.get(i).getX() <= player.getX() + 5 && 
				enemy.get(i).getY() >= player.getY() - 5 &&
				enemy.get(i).getY() <= player.getY() + 5 )
			{
				return true;
			}
	...
	public boolean enemyBulletHitPlayerCheck()//Check if player has been hit by enemy bullet
	{
		for( int i = enemyBullet.size() - 1; i >= 0; i-- )
		{
			if( enemyBullet.get(i).getX() + enemyBullet.get(i).getWidth()/2 >= player.getX() &&
				enemyBullet.get(i).getX() + enemyBullet.get(i).getWidth()/2 <= player.getX() + player.getWidth() && 
				enemyBullet.get(i).getY() + enemyBullet.get(i).getHeight()/2 >= player.getY() &&
				enemyBullet.get(i).getY() + enemyBullet.get(i).getHeight()/2 <= player.getY() + player.getHeight() )
			{
				return true;
			}
		}
	...
	public void enemyHitPlayerAvoid()//Make sure player doesn't spawn on an enemy
	{
		...
		for( int i = 0; i < enemy.size(); i++ )
		{
			if( enemy.get(i).getX() >= player.getX() - 31 &&
				enemy.get(i).getX() <= player.getX() + 31 && 
				enemy.get(i).getY() >= player.getY() - 31 &&
				enemy.get(i).getY() <= player.getY() + 31 )
			{
				player.setX( rand.nextInt( getWidth() ) );
				player.setY( rand.nextInt( getHeight() ) );
	...
	public void bulletMovement( LinkedList<Bullet> _bullet )//Movement of the bullets in four different directions
	{
	...
	public void humanMovement()//Movement of the humans in four different directions
	{
		for( int i = 0; i < human.size(); i++ )
		{
	...
	public void humanDirectionChange()//Change direction of human movement
	{
		for( int i = 0; i < human.size(); i++ )
		{
			...
			Random random = new Random();
			int temp = random.nextInt( 40 );//1/40 chance every frame of humans to change direction
			if( temp == 1 )
			{
				int temp2 = random.nextInt( 4 );
				switch( temp2 )
	...
	public void enemyDirectionChange()//Change of direction for type 2 enemies
	{
		for( int i = 0; i < enemy.size(); i++ )
		{
			if( enemy.get(i) instanceof Shooter )
			{
				...
				Random random = new Random();
				int temp = random.nextInt( 60 );//1/60 chance every frame of enemies to change direction
				if( temp == 1 )
				{
					int temp2 = random.nextInt( 4 );
					switch( temp2 )
